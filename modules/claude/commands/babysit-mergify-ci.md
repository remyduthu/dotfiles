...
Monitor CI status on a PR and automatically fix failures.
...
Use `AskUserQuestion` to ask the user for the PR URL. Store it for use in subsequent `gh` commands.
...
```bash
gh pr checks <PR_URL> --json name,state,bucket,link --jq '.[] | select(.bucket == "fail")' 2>&1
```
...
### Step 3: For each failed check, fetch logs
...
### Step 5: Amend the commit and push
